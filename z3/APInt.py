...
def count_lones(b, solver):
    return count_lzeros(~b, solver)


tmp_count_lones = count_lones(tmp, sol)


def count_rones(b, solver):
    return count_rzeros(~b, solver)


tmp_count_rones = count_rones(tmp, sol)
...
if res == sat:
    ...
    print("count_lones:", mod.eval(tmp_count_lones), "count_rones:", mod.eval(tmp_count_rones))
